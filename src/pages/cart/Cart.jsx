...
const Cart = () => {
  const navigate = useNavigate();
  const [cartItems, setCartItems] = useState([]);
  const [completeCart, setCompleteCart] = useState(null); // Changed initial state to null
  const [fetchCartLoader, setFetchCartLoader] = useState(true); // Start with true for initial load
  const [productLoaders, setProductLoaders] = useState();
  ...
  const confirmDelete = async () => {
    if (productToDelete) {
      ...
      setProductToDelete(null);
      setShowConfirmDialog(false);
      await fetchCartItems(); // Wait for the cart to refresh
    }
  };
